...
const { updateBackupDrive, serverGet, serverGetAsync, createBackupDirAsync, updateBackupDirsOrFilesAsync } = require('./server')
...
class Backup {
  constructor (drive, dirs) {
    ...
    this.checkTopDirAsync = async (entries, drv) => {
      ...
      const remoteTopDirs = listNav.entries.filter(e => !e.deleted)
      const backupDir = []
      const lackDirs = new Set(remoteTopDirs)
      for (let i = 0; i < entries.length; i++) {
        ...
        if (remoteEntry) {
          ...
          await updateBackupDirsOrFilesAsync(driveUUID, dirUUID, [{ bname, args }])
          backupDir.push({ driveUUID, dirUUID: uuid, entries: newEntries })

          lackDirs.delete(remoteEntry)
        } else {
          const attr = {
            bctime: stat.ctime.getTime(),
            bmtime: stat.mtime.getTime(),
      ...
      if (lackDirs.size) {
        const lackEntries = [...lackDirs].map(d => ({ bname: d.bname, args: { op: 'updateAttr', archived: true } }))
    ...
    this.checkTopDirAsync(purePaths, this.drive).then((backupDir) => {
      this.backup(backupDir, (error) => {
        ...
        if (!error) {
          ...
          updateBackupDrive(this.drive, { lastBackupTime: new Date().getTime() }, () => console.log('update backup dir success'))
        }
        this.status = 'Idle'
        this.run()
      })
  ...
  createTask (topDir, cb) {
    const { entries, driveUUID, dirUUID } = topDir
  ...
  backup (backupDir, cb) {
    ...
    const done = () => {
      ...
      i -= 1
      if (i > -1) {
        this.createTask(backupDir[i], done)
      } else cb()
    }
    done()
  }
  ...
  serverGet('drives', null, (error, drives) => {
    ...
    if (drive) {
      serverGet(`drives/${drive.uuid}/dirs/${drive.uuid}/`, null, (err, dirs) => {
        if (err) return
        const entries = dirs.entries.filter(e => !e.deleted && !e.archived)
