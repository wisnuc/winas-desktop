  ...
  startMonitor () {
    ...
    this.timer = setInterval(() => {
      ...
      if (this.lastTenData.length === 10) {
        const deltaSize = this.lastTenData[0].transferSize - this.lastTenData[9].transferSize
        const restTimeBySize = (data.size - data.completeSize) / deltaSize * 9

        const deltaCount = this.lastTenData[0].finishCount - this.lastTenData[9].finishCount
        const restTimeByCount = (data.count - data.finishCount) / deltaCount * 9
        ...
        // max restTime: 3 days
        restTime = Math.min(restTime, 259200)
        ...
        console.log('delta >>', deltaSize, deltaCount)
