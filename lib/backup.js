...
class Backup {
  constructor (drive, dirs) {
    ...
    this.readUploadInfoAsync = async (entries, dirUUID, driveUUID, domain) => {
  ...
  start () {
    console.log('Backup start', this.drive, this.dirs)
    this.startMonitor()
    this.status = 'Idle'
    this.dirty = true
    this.diff()
  ...
  diff () {
    if (this.status !== 'Idle' || !this.dirty) return
    this.status = 'diff'
    this.dirty = false
    const purePaths = this.dirs.map(d => d.localPath)
    this.readUploadInfo(purePaths, this.drive.uuid, this.drive.uuid, 'backup')
    /* after this backup action finished */
    setTimeout(() => {
      this.status = 'Idle'
      this.diff()
    }, 10 * 1000)
  ...
  startMonitor () {
    this.dirs.forEach((dir) => {
      const watcher = fs.watch(path.resolve(dir.localPath), { recursive: true }, () => {
        this.dirty = true
        this.diff()
      })
  ...
  readUploadInfo (entries, dirUUID, driveUUID, domain) {
    this.readUploadInfoAsync(entries, dirUUID, driveUUID, domain)
      .then((count) => {
