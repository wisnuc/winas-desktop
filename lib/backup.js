...
class Backup {
    ...
    this.checkTopDirAsync(purePaths, this.drive).then((backupDir) => {
      this.backup(backupDir, (error) => {
        ...
        if (!error) {
          ...
          updateBackupDrive(this.drive, { status: 'Idle', lastBackupTime: this.lastBackupTime }, () => console.log('Backup Idle'))
  ...
  summary () {
    ...
    return ({ count, finishCount, size, completeSize, status: this.status, lastBackupTime: this.lastBackupTime })
  }
...
const onCommand = (event, name, args) => {
  ...
  switch (name) {
    ...
    case 'updateBackupDrive':
      updateBackupDrive(args.drive, args.attr, (err, res) => onRes(args.session, err, res))
      break
    default:
      break
  }
