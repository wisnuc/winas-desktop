...
class Backup {
  ...
  retry () {
    this.retryCount += 1
    console.log('Backup Failed, retry', this.retryCount)
    this.status = 'Failed'
    this.retryTimer = setTimeout(() => this.run(), 1000 * 60 * this.retry ** 2) // retry later
  }
  ...
  run () {
    ...
    updateBackupDrive(this.drive, { status: 'Working' }, (er, res) => {
      if (er) {
        ...
        this.status = 'Idle'
        this.dirty = true
        setTimeout(() => this.run(), 10000) // 10 second later
        return
      }
    ...
    const task = new Task(entries, driveUUID, dirUUID, (errors, warnings) => {
      ...
      const args = {
        ...
        metadata: {
          ...
          disabled: false,
          status: 'Idle',
          lastBackupTime: task.hasFileUpload ? new Date().getTime() : lastBackupTime
        }
      }
  ...
  backup (backupDir, cb) {
    console.log('backup start', backupDir)
    let i = backupDir.length
    const errorList = []
    const warningList = []
    const done = (errors, warnings) => {
      console.log('backup done', backupDir.length, i)
