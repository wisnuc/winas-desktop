...
const { updateBackupDrive, serverGet, serverGetAsync, createBackupDirAsync, updateBackupDirsOrFiles, updateBackupDirsOrFilesAsync } = require('./server')
...
class Backup {
    ...
    this.checkTopDirAsync(purePaths, this.drive).then((backupDir) => {
      this.backup(backupDir, (error) => {
        ...
        if (!error) {
          ...
          updateBackupDrive(this.drive, { status: 'Idle', lastBackupTime: new Date().getTime() }, () => console.log('Backup Idle'))
  ...
  startMonitor () {
    ...
    this.timer = setInterval(() => {
      ...
      getMainWindow().webContents.send('BACKUP_MSG', { restTime, ...data })
    }, 1000)
  }
  ...
  summary () {
    ...
    return ({ count, finishCount, size, completeSize, status: this.status })
  }
