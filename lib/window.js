...
const WINDOW_WIDTH = env ? 900 : 450
...
ipcMain.on('LOGIN', () => {
  ...
  win.hide()
  win.setBounds({ x: bounds.x - 255, y: bounds.y - 110, width: 960, height: 720 }, true)
  // win.setSize(960, 720, true)
  setTimeout(() => win.show(), 300)
})
...
ipcMain.on('LOGOUT', () => {
  ...
  win.setBounds({
    x: r(bounds.x + 255 * p),
    y: r(bounds.y + 110 * p),
    width: r(960 + (450 - 960) * p),
    height: r(720 + (500 - 720) * p)
  }, true)
  setTimeout(() => win.show(), 300)
})
