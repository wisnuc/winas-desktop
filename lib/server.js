...
class UploadMultipleFiles {
  ...
  normalUpload () {
    console.log('normalUpload', this.Files.map(f => f.entry))
    ...
    this.Files.forEach((file) => {
      ...
      for (let i = 0; i < parts.length; i++) {
        ...
        if (policy && policy.archiveRemote) { // backup
          const args = { op: 'archive', hash: policy.remoteHash, uuid: policy.remoteUUID }
...
const createBackupDir = (driveUUID, dirUUID, dirname, attr, callback) => {
  ...
  handle.end((err, res) => {
    const { error, body } = parseRes(err, res)
    console.log('mkdir res', err, body)
    let dirEntry
    let e = error
    if (!error) {
      dirEntry = body && body[0] && body[0].data
...
const updateBackupDir = (drive, props, cb) => {
  serverGet(`drives/${drive.uuid}`, null, (err, body) => {
    console.log('updateBackupDir', err, body)
    const { client } = body
    Object.assign(client, { lastBackupTime: new Date().getTime() })
    apatch(`drives/${drive.uuid}`, { op: 'backup', client }).end(cb)
  })
}

const archiveRemoteDirsOrFiles = (driveUUID, dirUUID, Files, callback) => {
  const handle = apost(`drives/${driveUUID}/dirs/${dirUUID}/entries`, null, true)
  Files.forEach((file) => {
    const { uuid, hash, bname } = file
    const args = { op: 'archive', hash, uuid }
...
const archiveRemoteDirsOrFilesAsync = Promise.promisify(archiveRemoteDirsOrFiles)

module.exports = {
  archiveRemoteDirsOrFiles,
  archiveRemoteDirsOrFilesAsync,
  createBackupDir,
