...
class Task {
  constructor (localEntries, driveId, dirId, onFinished) {
        ...
        const read = async (entries, dirUUID, driveUUID, task) => {
          ...
          for (let i = 0; i < entries.length; i++) {
            ...
            if (stat.isDirectory()) {
              ...
              if (!remoteDir) {
                ...
                task.warnings.push({
                  pipe: 'diff', entry, error: { code: 'EDELDIR' }, name: fullName, type: 'directory', isWarning: true
                })
              }
              if (uuid) {
                /* read child */
          ...
          remoteFiles.forEach((r) => {
            ...
            if (value) {
              ...
              if (r.deleted) {
                task.warnings.push({
                  pipe: 'diff', entry: value.entry, error: { code: 'EDELFILE' }, name: value.name, type: 'file', isWarning: true
                })
              }
            }
          })
    ...
    this.readDir.on('step', () => {
      ...
      if (this.readDir.isStopped()) this.finish()
    })
  }
