...
const { createBackupDirAsync, UploadMultipleFiles, serverGetAsync, archiveRemoteDirsOrFilesAsync } = require('./server')
    ...
    this.diff = new Transform({
      ...
      transform (x, callback) {
        const read = async (entries, dirUUID, driveUUID, task) => {
          ...
          await archiveRemoteDirsOrFilesAsync(driveUUID, dirUUID, [...lackFiles, ...lackDirs])
